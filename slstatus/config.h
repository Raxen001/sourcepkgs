...
static const struct arg args[] = {
	...
	{ ram_total    , "/%s          ", NULL},
// 	{ run_command  , "^b#B16286^^c#1d2021^                    %s                   ", "amixer -c 1 -M sget Master | awk -F\"[][]\" '/%/ { print $2 }' | head -n1 | sed 's/.$//'" },
//   { run_command  , "^b#d79921^^c#1d2021^                    %s                   ", "brightnessctl -d 'amdgpu_bl0' -m | awk -F ',' '{ print $4 }' | sed 's/.$//' "},
